...
import com.guce.application.domain.port.in.AgentGetFimexRequestByIdUseCase;
...
@RestController
@RequestMapping("/api/v1/fimex/agent/requests")
@RequiredArgsConstructor
@Tag(name = "FIMEX Agent - Demandes", description = "API de traitement (Agents)")
@PreAuthorize("hasRole('FIMEX_AGENT')")
public class AgentFimexRequestController {

    private final AgentGetFimexRequestByIdUseCase agentGetFimexRequestByIdUseCase;
    private final AgentSearchFimexRequestsUseCase agentSearchFimexRequestsUseCase;
    ...
    public ResponseEntity<FimexRequestDetailDTO> getRequest(@PathVariable UUID id) {
        FimexRequestModel request = agentGetFimexRequestByIdUseCase.execute(id);
    ...
    public ResponseEntity<Page<FimexRequestListDTO>> searchRequests(
        ...
        SearchCriteria criteria = new SearchCriteria(
                ...
                page,
                size);
        Page<FimexRequestListDTO> results = agentSearchFimexRequestsUseCase.execute(criteria).map(webMapper::toListDTO);
